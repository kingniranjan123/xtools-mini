...
def apply_watermark_to_folder(folder, watermark_path, position='BR',
                               opacity=0.75, scale=0.15,
                               output_mode='new_folder', progress_cb=None):
    """
    Apply watermark to all .mp4 files in `folder`.
    ...
    if output_mode == 'new_folder':
        ...
        out_dir = folder  # overwrite in place (backup handled by _ffmpeg_watermark)
    ...
    for idx, fname in enumerate(mp4_files, 1):
        ...
        try:
            _ffmpeg_watermark(src, dst, watermark_path, pos_expr, opacity, scale)
            if output_mode == 'overwrite':
                os.replace(dst, src)
            count += 1
        except Exception as exc:
...
def _ffmpeg_watermark(src, dst, wm_path, pos_expr, opacity, scale):
    """Apply a single watermark using FFmpeg filter_complex."""
    # Scale watermark to (scale * video_width) with maintained aspect ratio
    vf = (
        f"[1:v]scale=iw*{scale:.4f}:-1,format=rgba,"
        f"colorchannelmixer=aa={opacity:.4f}[wm];"
        f"[0:v][wm]overlay={pos_expr}"
